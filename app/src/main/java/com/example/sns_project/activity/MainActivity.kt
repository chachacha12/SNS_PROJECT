...
class MainActivity : BasicActivity() {
    ...
    val onPostListener = object : OnPostListener {
        override fun onDelete(position: Int) {        //postList상에서의 게시글 위치값을 받아서 지워줄거임
            ...
            //이미지,영상등이 스토리지에 저장된 이름을 알아내서 스토리지에서 삭제하는 로직  // 파이어베이스의 스토리지는 여러 사진들 모아두는 폴더가 따로 없어서 사진 하나하나 지워줘야한다고 함.
        ...
        override fun onModify(position: Int) {
            var id = postList.get(position).id!!
            myStartActivity(WritePostActivity::class.java, id)  //선택한 게시물의 id값을 실어서 인텐트를 보내줌
    ...
    fun myStartActivity(c: Class<*>, id: String) {
        var i = Intent(this, c)
        i.putExtra("id", id)
        startActivity(i)
    }
